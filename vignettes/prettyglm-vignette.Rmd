...
or return a dataframe by setting `return_data` to `TRUE`.

`pretty_coefficients()` also supports parsnip and workflow model objects which use the `glm` model engine.
