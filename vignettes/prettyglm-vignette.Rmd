...
columns_to_factor <- c('Pclass', 'Sex', 'Cabin', 'Embarked', 'Cabintype')
...
a_custom_predict_function <- function(target, model_object, dataset) {
  ...
  Actual_Values <- dplyr::pull(dplyr::select(
    dataset,
    tidyselect::all_of(c(target))
  ))
  if (class(Actual_Values) == 'factor') {
    Actual_Values <- base::as.numeric(as.character(Actual_Values))
  }
  Predicted_Values <- base::as.numeric(stats::predict(
    model_object,
    dataset,
    type = 'response'
  ))

  to_return <- base::data.frame(
  ...
  to_return <- to_return %>%
    dplyr::mutate(
      Predicted_Values = base::ifelse(
        Predicted_Values > 0.4,
        0.4,
        Predicted_Values
      )
    )
