...
clean_coefficients <- function(
  ...
  x <- m %>%
    ...
    dplyr::mutate(
      variable = stringr::str_extract(
        string = name,
        pattern = tidycat::factor_regex(m)
      ),
      level = stringr::str_remove(
        string = name,
        pattern = tidycat::factor_regex(m)
      ),
      ...
      effect = base::ifelse(
        test = stringr::str_detect(string = variable, pattern = ":"),
        yes = base::ifelse(
          stringr::str_detect(string = level, pattern = ":", negate = TRUE),
  ...
  number_of_rows <- x %>%
    ...
    dplyr::summarise(number_of_rows = dplyr::n(), .groups = "drop")
  x <- x %>%
    dplyr::left_join(number_of_rows, by = "variable") %>%
    dplyr::mutate(
      effect = base::ifelse(
        effect == 'otherinteraction',
        yes = base::ifelse(
          number_of_rows == 1,
  ...
  for (i in 1:length(x$variable)) {
    if (x$variable[i] == '(Intercept)') {
      ...
      if (x$effect[i] == 'factorandctsinteraction') {
        ...
        if (
          methods::is(
            dplyr::pull(dplyr::select(m$data, tidyselect::all_of(var1))),
            'factor'
          ) ==
            TRUE
        ) {
        ...
        if (
          methods::is(
            dplyr::pull(dplyr::select(m$data, tidyselect::all_of(var2))),
            'factor'
          ) ==
            TRUE
        ) {
        ...
        for (k in 1:base::length(base::unlist(base::strsplit(
      ...
      x$variable[i] %in% names(which(attr(m$terms, "dataClasses") == 'numeric'))
    ) {
  ...
  if (is.null(spline_seperator) == F) {
    x <- x %>%
      ...
      dplyr::mutate(
        effect = base::ifelse(
          (effect == 'factorandctsinteraction') &
            (stringr::str_detect(
              string = variable,
              pattern = spline_seperator
            ) ==
  ...
  if (vimethod == 'model') {
    ...
    x <- dplyr::left_join(x, v, by = c('term' = 'Variable')) %>%
      dplyr::mutate(
        Importance = base::ifelse(is.na(Importance), 0, Importance),
    ...
    ivp <- vp %>%
      ...
      dplyr::mutate(
        Interacted_Variable = base::paste0(Variable.x, ':', Variable.y)
      ) %>%
